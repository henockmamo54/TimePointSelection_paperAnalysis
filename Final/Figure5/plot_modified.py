...
rect=patches.Rectangle(( min(temp_data['Labeling Duration']) - 0.4, low-0.001),
                        max(temp_data['Labeling Duration']) -min(temp_data['Labeling Duration']) +0.7, 
                        high-low +0.004, facecolor="g", alpha=0.2)
...
ax.text(17, low, "Lower Limit (theo.) = " +r"$0.65 * I_0(0) + 0.35 * I_0^{asmyp}$", ha='center', va='bottom', fontsize=10, color='m')
ax.text(17, high, r"Upper Limit (theo.) = $I_0(0) - 0.031$", ha='center', va='bottom', fontsize=10, color='k')
